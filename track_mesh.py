...
from flame.BlendModel import Blend
...
class Tracker(object):
    ...
    def optimize_camera(self, batch, steps=10000):
    ...
    def checkpoint2(self, batch, visualizations=[[View.GROUND_TRUTH, View.LANDMARKS, View.HEATMAP], [View.COLOR_OVERLAY, View.SHAPE_OVERLAY, View.SHAPE]], frame_dst='/blendvideo', save=False, dump_directly=False):
        batch = self.to_cuda(batch)
        images, landmarks, landmarks_dense, _, _ = self.parse_batch(batch)
        exp_params = batch['blend']
        ...
        with torch.no_grad():
            ...
            vertices = self.blend(exp_params)
    ...
    def optimize_mesh(self):
        ...
        self.blend = Blend(self.config, self.actor_name)
        
        # self.blend.saveData()
        # self.blend.saveBlend()
        
        #self.blend.loadData(load_file = 'BlendModel_2.npz')
        
        self.blend.to(self.device)
        
        # for i in range(51):
        #     exp = torch.zeros((1, 51)).to(self.device)
        #     exp[0,i] += 1
        #     data = self.blend(exp)
        #     self.blend.saveBlend(vertices = data[0], save_file=f'BlendObj/{i}.obj')
        # embed()
        
        #10-4 0.002 0.00066
        #10-6 0.008 
        #10-5 0.002
        params = [{'params': [self.blend.canonical, self.blend.exp_face], 'lr': 0.00001}]
        optimizer = torch.optim.Adam(params)
        ...
        for t in range(3):
            tot_loss = 0
            f =  len(self.dataset) - 500
            
            for i in list(range(0, len(self.dataset) - 500)):
                batch = self.to_cuda(self.dataset[i], unsqueeze=True)
                if type(batch) is torch.Tensor:
                    continue
                ...
                vertices, lmk68, lmkMP = self.flame(
                    #cameras=torch.inverse(self.cameras.R),
                    cameras=torch.inverse(rotation_6d_to_matrix(self.R)),
                    shape_params=self.shape,
                    expression_params=self.exp,
                    eye_pose_params=self.eyes,
                    jaw_pose_params=self.jaw,
                ...
                mp_gt = vertices[:, mediapipe_idx,:]
                
                vertices_b = self.blend(exp_params)
                mp_pr = vertices_b[:,mediapipe_idx,:]

                losses['l2_loss'] = nn.MSELoss()(vertices_b, vertices)
                losses['mp_loss'] = nn.MSELoss()(mp_gt, mp_pr) * 10
                ...
                tot_loss += all_loss.item()
                
                # if t>2:
                #     self.checkpoint2(batch, visualizations=[[View.GROUND_TRUTH, View.COLOR_OVERLAY, View.LANDMARKS, View.SHAPE]])
                
            eval_loss = 0
            for i in list(range(len(self.dataset) - 500, len(self.dataset))):
                batch = self.to_cuda(self.dataset[i], unsqueeze=True)
                if type(batch) is torch.Tensor:
                    continue
                self.load_checkpoint(i)
            ...
            print(f'Train loss {tot_loss}, Eval loss {eval_loss}')
            if t > 2 and eval_loss < best_loss:
                print(t)
                best_loss = eval_loss
                self.blend.saveData(save_file = config.save_folder +  self.actor_name + "/blendData.npz")
                self.blend.saveBlend(vertices = vertices_b[0])
