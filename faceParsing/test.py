...
class ImagesDataset(Dataset):
    ...
    def __getitem__(self, index):
        ...
        blend_path = imagepath.replace('images', 'blend').replace('png', 'npz').replace('jpg', 'npz')
        ...
        payload = {
            ...
            'shape': shapes,
            'lmk_b': torch.from_numpy(blend['lmk']).float(),
            'blend': torch.from_numpy(blend['blend'][1:]).float(),
            'matrix': torch.from_numpy(blend['matrix']).float()
        }
